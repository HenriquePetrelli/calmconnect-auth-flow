-- Group testimonials no longer get auto-deleted once they hit 10 "não
-- gostei". That trigger behavior let any group of users permanently remove
-- another patient's testimonial with no moderation, no notice to the
-- author, and no soft-delete — a real abuse/harassment vector in a
-- mental-health app. Testimonials now stay, flagged for review; an admin
-- decides (edit or delete) manually.
...
-- Admin moderation queue: every testimonial, author name only when not
-- anônimo, sorted with >=10-dislike ones first (the manual replacement for
-- the old auto-delete threshold).
