-- Add rejected_at column to psychologist_registrations table if not exists
DO $$ 
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                   WHERE table_name = 'psychologist_registrations' 
...
DECLARE
  doc_path text;
BEGIN
  -- Get document path before deletion for cleanup
