-- Per-date exceptions on top of the recurring weekly schedule
-- (psychologist_availability): a psychologist can block a slot that would
-- normally be open on a specific date ('bloqueio'), or open an extra slot
-- outside their usual pattern for a specific date ('abertura'), without
-- touching the recurring template. Mirrors the RLS shape of
-- psychologist_availability: everyone can read (needed for patient
-- scheduling), only the owning psychologist can write.
