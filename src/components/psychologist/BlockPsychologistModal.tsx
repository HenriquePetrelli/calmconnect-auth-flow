...
export const BlockPsychologistModal = ({
  open,
  onOpenChange,
  psychologistId,
  psychologistName,
  ...
  const submit = async () => {
    ...
    setSaving(true);
    try {
      const { data, error } = await supabase.functions.invoke('admin-block-psychologist', {
        body: {
      ...
      <DialogContent className="max-w-md">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            {mode === 'block' ? 'Bloquear psicólogo' : 'Desbloquear psicólogo'}
          </DialogTitle>
          <DialogDescription>
            {mode === 'block'
              ? `Defina o período e o motivo do bloqueio de ${psychologistName}.`
              : `Confirme o desbloqueio de ${psychologistName}.`}
          </DialogDescription>
        </DialogHeader>
