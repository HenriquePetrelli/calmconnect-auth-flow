...
/** Faixa "de-até" do horário-padrão de um dia, só para exibição e como range
 * da grade de bloqueio — NUNCA editada aqui. Um dia com mais de um bloco no
 * padrão (ex.: manhã e tarde separadas) é representado pela faixa mais
 * ampla que os cobre; editar isso de fato é só em "Minha agenda". */
...
const derivePatternByDay = (blocks: AvailabilityBlock[]): Record<number, DayPattern> => {
  ...
  for (const day of DAYS_DISPLAY_ORDER) {
    ...
    if (dayBlocks.length === 0) {
      pattern[day] = { hasSchedule: false, start: '', end: '' };
    } else {
      const starts = dayBlocks.map((b) => b.start_time).sort();
      const ends = dayBlocks.map((b) => b.end_time).sort();
      pattern[day] = { hasSchedule: true, start: starts[0], end: ends[ends.length - 1] };
    }
  }
  return pattern;
};
...
/**
 * Confirmação semanal: mostra o horário-padrão (só leitura — editar é em
 * "Minha agenda") e deixa bloquear/abrir horários pontuais SÓ para essa
...
export const WeeklyScheduleModal: React.FC<WeeklyScheduleModalProps> = ({ open, onClose, onConfirmed }) => {
  ...
  const patternByDay = useMemo(() => derivePatternByDay(baseBlocks), [baseBlocks]);
        ...
        {loading ? (
          ...
          <div className="space-y-3">
            {dates.map((date) => {
              ...
              const pattern = patternByDay[dayOfWeek] ?? { hasSchedule: false, start: '', end: '' };
              const dayOverrides = overridesByDate[date] ?? [];
              const aberturas = dayOverrides.filter((o) => o.type === 'abertura');
              const gridSlots = pattern.hasSchedule ? halfHourGridSlots(pattern.start, pattern.end) : [];
                ...
                <div key={date} className="rounded-lg border p-3">
                  <div className="flex items-center justify-between gap-2 flex-wrap">
                    ...
                    <span className="text-xs text-muted-foreground">
                      {pattern.hasSchedule ? `${pattern.start} às ${pattern.end}` : 'Sem atendimento padrão'}
                    </span>
                  </div>
                    ...
                    <Collapsible open={isExpanded} onOpenChange={(v) => setExpandedDay(v ? dayOfWeek : null)} className="mt-2">
                      ...
                      <CollapsibleContent className="mt-2 space-y-2">
                        <p className="flex items-center gap-1 text-xs text-muted-foreground">
                          ...
                          Horários em verde estão disponíveis. Clique para bloquear ou liberar um horário — só
                          nesta semana.
                        </p>
