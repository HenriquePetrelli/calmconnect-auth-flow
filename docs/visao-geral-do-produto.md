...
- Confirmação semanal: ao entrar no painel, o psicólogo vê uma vez por semana (e pode reabrir a qualquer momento pelo ícone no cabeçalho) uma modal (`WeeklyScheduleModal`) com um card por dia dos próximos 7 dias — inclusive na primeira vez que usa o app, já que ainda não há semana confirmada. Cada dia tem um switch (atende/não atende) e um range de horário (início/fim), pré-preenchido a partir do horário-padrão salvo (editável a qualquer momento em "Minha agenda", `/psychologist-availability`). Dentro do card, "Personalizar horários" abre uma grade de meia em meia hora dentro do range do dia, com os horários disponíveis em verde por padrão — clicar num horário disponível bloqueia os 30 minutos seguintes (sai do verde); clicar de novo libera (volta ao verde); horários com consulta já marcada aparecem travados (não clicáveis). Ainda no card, "Adicionar horário extra" cria uma abertura pontual fora do range do dia (efeito imediato). Um único botão "Confirmar horários livres da semana" no rodapé salva tudo de uma vez: só grava o horário-padrão (`set_psychologist_availability`) se algum range/dia mudou, e só grava os bloqueios pontuais (`psychologist_availability_overrides`, em lote via `applyChanges`) com a diferença entre o que já estava salvo e o que foi clicado na grade.
