...
- 2026-09-08 — Nova rodada de levantamento (agora em pagamentos/Stripe e nas funções de manutenção ainda não revisadas), com correção de todos os achados:
  - **`cleanup-user` sem autenticação nenhuma**: a edge function lia `userId` direto do corpo da requisição e nunca checava quem estava chamando — qualquer requisição com um UUID válido apagava a conta completa de qualquer usuário (documentos no storage, linhas em `psychologist_registrations`/`psychologists`/`profiles` e a conta de autenticação em si). O único uso legítimo é o rollback de um cadastro de psicólogo que falhou no meio do caminho (`cleanupFailedSignup`, chamado com a sessão de quem acabou de se cadastrar). Adicionada autenticação via `Authorization` header e checagem `userId === user.id` — a função só apaga a própria conta de quem chama.
  - **Código morto: `plan-cancel` + tabela `subscriptions` inexistente**: função cancelava assinatura contra uma tabela `subscriptions` que nunca existiu em nenhuma migração (o modelo real usa `subscribers` + Stripe); nunca era chamada de lugar nenhum do app (o cancelamento real usa `cancel-subscription`, que já funciona). Removida por completo, junto da entrada em `config.toml`.
  - **`customer-portal` pronto mas nunca ligado a nada**: a função já abria corretamente o Stripe Billing Portal, mas não existia botão nenhum chamando ela. Adicionado botão "Gerenciar Pagamento" em `/subscription-plans` (visível junto ao de downgrade, só para quem já tem assinatura ativa).
  - Também revisado `gerenciar-conversas` (função de manutenção que expira conversas antigas): não tem autenticação, mas não recebe nenhum parâmetro do usuário — mesmo padrão já aceito em outras funções tipo cron do projeto (`reset-weekly-goals`, `psychologist-cleanup`); mantida como está.
  - Validado com typecheck, build de produção e suíte completa (as mesmas 3 falhas pré-existentes em `availableTimeSlots.test.ts` continuam — dependem da data corrida do dia do teste e já falhavam antes desta rodada, sem relação com as mudanças feitas aqui).
- 2026-09-08 — Corrigida a falha de segurança mais grave encontrada em toda a auditoria desta sessão:
  - **`is_super_admin()` tinha uma porta dos fundos por `user_metadata`**: a função usada por praticamente todo o controle de acesso admin (RLS de `appointments`, `emergency_requests`, `profiles`, `security_audit_log`, `admin_audit_log`, `psychologist_registrations`, e as edge functions `payment-sync`, `admin-psychologist-management`, `confirm-payment`, `psychologist-management`, `admin-block-*`, `admin-delete-*`, `admin-update-*`) aceitava como super admin tanto quem estava na tabela `admin_users` quanto qualquer conta com `user_metadata.is_super_admin = true` — campo que o próprio usuário controla via `supabase.auth.updateUser({ data: { is_super_admin: true } })`. Qualquer paciente ou psicólogo cadastrado normalmente conseguia se autopromover a super admin com uma chamada no console do navegador. Removido o fallback por `user_metadata`; `is_super_admin()` passa a confiar só em `admin_users`, que é a via correta descrita em `docs/creating-an-admin-account.md`. Migração inclui uma trava que aborta (transação inteira, não só um aviso) se não existir nenhuma linha ativa em `admin_users` no momento de aplicar — evita que a correção tire o acesso de todo mundo por engano caso a conta admin real ainda dependesse só da metadata.
  - **`psychologist-registration-management` duplicava o mesmo furo direto no código** (checava `user.user_metadata?.is_super_admin` sem passar pela função) e, além disso, nunca era chamado por nenhuma tela do app (o fluxo real de aprovação usa `psychologist-management`, que já valida corretamente). Removida por completo, mesmo padrão já aplicado a `create-admin-account`.
  - **Correção de documentação**: `docs/creating-an-admin-account.md` citava uma rota `/admin-login` que nunca existiu no app; corrigido para descrever o fluxo real (login normal, redirecionado pelo `user_type` do perfil).
  - Validado com Postgres local simulando os dois cenários (correção bloqueada com `admin_users` vazia; aplicada com sucesso e furo fechado assim que existe uma linha ativa), typecheck, build de produção e suíte completa.
- 2026-09-08 — Adicionado um mascote (preguiça) ao app, em vários pontos-chave, no estilo Duolingo:
  - Novo sistema em `src/components/mascot/` — um `<Mascot pose="..." />` genérico com 5 poses (`wave`, `sleep`, `celebrate`, `thinking`, `hug`), cada uma um SVG desenhado à mão (sem dependência de imagem externa). Trocar o animal do app inteiro no futuro é criar uma pasta nova em `species/<nome>` com as mesmas 5 poses e mudar uma única constante (`ACTIVE_SPECIES` em `Mascot.tsx`) — nenhum dos lugares que usam `<Mascot pose="..." />` precisa mudar.
  - Colocado em: tela de login (acenando, ao lado do logo), estado vazio de "Notificações" (cochilando), estado vazio de "Conquistas" (pensativo, com uma interrogação), modal de conquista desbloqueada (comemorando, com o ícone específico da conquista como selo sobreposto), tela de espera do SOS (abraçando um coração, como companhia enquanto busca um profissional), card de introdução da Respiração Guiada (abraçando um coração, no lugar do ícone genérico de vento) e na tela de sessão concluída da respiração (comemorando, no lugar do ícone de check).
  - Validado visualmente renderizando as 5 poses com Playwright antes de integrar (duas tiveram os braços redesenhados na frente do corpo — antes ficavam escondidos atrás da barriga), além de typecheck, build de produção e suíte completa.
- 2026-09-08 — Trocado o ícone do botão de SOS (nav inferior mobile e item "Ajuda Emergencial" do menu lateral desktop): era um sol (`Sun`, sem relação nenhuma com emergência), agora é uma bóia salva-vidas (`LifeBuoy`) — símbolo universal de ajuda/resgate, sem o tom alarmante de um ícone de alerta/sirene. Validado visualmente com Playwright no contexto real dos dois botões, além de typecheck, build de produção e suíte completa.
- 2026-09-08 — Ajustes visuais no botão/menu de SOS, a pedido:
  - No menu lateral desktop, o ícone da bóia ficou maior (`w-5 h-5` → `w-7 h-7`), mais fácil de reconhecer.
  - Na navegação inferior (mobile/tablet), o item de SOS deixou de ter o círculo laranja preenchido atrás do ícone — agora é só a bóia, maior que os outros ícones e colorida em laranja pra se destacar, com o mesmo padrão visual (rótulo, indicador de aba ativa) dos outros itens.
  - A barra de navegação inferior inteira foi modernizada: de uma barra reta grudada nas bordas da tela para uma barra flutuante com cantos arredondados, sombra suave e leve espaçamento das bordas — mantendo o tratamento de área segura (`safe-area-inset-bottom`) para iPhones com indicador de home.
  - Validado renderizando o componente de verdade (com o CSS real do app) via Playwright em mobile e desktop, além de typecheck, build de produção e suíte completa.
- 2026-09-08 — Segunda rodada de ajuste no botão de SOS, a pedido (a primeira não agradou):
  - **Ícone da bóia trocado por um desenho próprio** (`src/components/icons/LifeRingIcon.tsx`): a `LifeBuoy` do lucide-react era abstrata demais pra ler como bóia num ícone pequeno. O novo SVG desenha o anel clássico de resgate — 4 faixas alternadas laranja/branco, contorno escuro, buraco no centro e as 4 alças de corda — usado só na navegação inferior.
...
- 2026-09-11 — Correção de uma edição feita pelo Lovable direto no projeto (fora desta sessão) que não ficou como o esperado:
  - **Botão de SOS da navegação inferior não ficava centralizado de verdade**: a centralização dependia de `left-1/2` + `-translate-x-1/2` dentro da própria célula do grid, matematicamente correto mas frágil. Reescrito para o botão ficar fora do grid, centralizado direto na barra inteira via `left-0 right-0 mx-auto` (a célula do meio agora só reserva o espaço, vazia) — testado programaticamente (não só visualmente) em 6 larguras de tela diferentes (320 a 768px), com 0px de desvio do centro em todas.
  - **Botão "Ajuda Emergencial" do menu lateral desktop**: removido o gradiente, a animação de pulso (`animate-ping`) e o subtítulo "Toque para suporte imediato" que a edição do Lovable tinha adicionado — o subtítulo estourava a largura do botão e cortava o texto. Voltou a ser um bloco sólido (`bg-primary`, sem gradiente), com bom espaçamento, ícone grande dentro de um círculo branco (bom contraste) e uma única linha de texto.
  - Validado com Playwright (incluindo a medição de centralização por código), typecheck, build de produção e suíte completa.
- 2026-09-11 — Nova rodada de ajuste no botão de SOS da navegação inferior, a pedido:
  - **Barra sempre fixa no rodapé**: voltou a `bottom: 0` edge-to-edge (sem a margem/flutuação de 12px introduzida na rodada anterior), com cantos arredondados só na parte de cima.
  - **Posicionamento do botão trocado de `transform: translateY()` para `top` fixo em pixel**: em vez de `top-0` + `-translate-y-1/2`, agora é `top: -43px` direto — mesmo resultado visual (metade do círculo pra fora, metade apoiada na barra), mas sem depender de transform.
  - **Geometria crítica do botão (posição, largura, altura) movida de classes Tailwind com valor arbitrário (`top-[-43px]`, `h-[86px]`, `w-[86px]`) para `style` inline**: a suspeita é que o pipeline de build usado fora desta sessão (Lovable) não estava gerando essas classes de valor arbitrário corretamente, causando a divergência entre o que era validado aqui e o que aparecia no preview do usuário. Estilo inline elimina essa dependência por completo.
  - Validado renderizando o componente real via Playwright, typecheck, build de produção e suíte completa.
- 2026-09-11 — Últimos dois ajustes de precisão no botão de SOS, a pedido:
...
- 2026-09-14 — Levantamento de gaps em áreas ainda não revisadas (WebRTC e cadastro), com correção dos achados de código:
  - **Cadastro de psicólogo aceitava CPF incompleto/inválido no formulário**: diferente do cadastro de paciente (que já valida o dígito verificador de verdade), o de psicólogo só checava `min(11)` no campo mascarado — um CPF pela metade como `111.111.111` passava (11 caracteres contando pontuação, mas sem os 2 dígitos verificadores). Só a constraint do banco barrava, com uma mensagem técnica do Postgres vazando pro usuário. Extraída a validação de CPF (que só existia duplicada dentro de `SignUpForm.tsx`) para `src/utils/cpf.ts`, reaproveitada nos dois formulários; adicionada tradução amigável da mensagem de constraint como reforço.
  - **Limpeza de código morto**: removidas `sosItem`/`SosIcon` de `BottomNavigation.tsx`, variáveis que sobraram de uma reescrita anterior do componente e não eram mais usadas.
  - **Ainda não corrigido, decisão pendente**: a stack de WebRTC (chamadas de vídeo do SOS e de consultas) usa só servidores STUN, sem TURN — pacientes/psicólogos atrás de redes mais restritivas (CGNAT, rede corporativa) não conseguem completar a chamada, mesmo com a lógica de reconexão já existente. Requer contratar/configurar um provedor de TURN (custo + credencial), decisão do usuário.
  - Validado com typecheck, build de produção, suíte completa e um teste de sanidade da função de CPF extraída (CPF incompleto agora rejeitado, CPF válido continua aceito).
- 2026-09-14 — Continuação do levantamento, com correção de mais dois achados (sem custo adicional, excluindo de propósito o gap de TURN do WebRTC, que fica pendente de decisão):
  - **Consulta que expira sem resposta do psicólogo (24h) não avisava o paciente**: um recuso manual já dispara notificação in-app + email (via `send-appointment-notification`), mas `auto-decline-appointments` (a rotina que expira pedidos parados) só atualizava o status pro banco, silenciosamente — o paciente só descobria checando o app. Agora dispara a mesma notificação usada no recuso manual, best-effort (uma falha de notificação não impede o auto-decline em si).
  - **CORS quebrado em `mark-sos-used`**: única função do projeto com uma lista fixa de domínios permitidos (URLs antigas de preview do Lovable + localhost) em vez do padrão `'*'` usado em todo o resto do código. Qualquer chamada vinda de um domínio fora dessa lista — incluindo o domínio de produção real, provavelmente — recebia de volta um `Access-Control-Allow-Origin` que não batia com a origem, e o navegador bloqueava a resposta silenciosamente (a chamada já é best-effort, dentro de um try/catch que só loga no console). Na prática, a marcação de uso mensal do SOS (plano Plus, 1x/mês) pode nunca ter funcionado em produção. Corrigido para `'*'`, mesmo padrão do resto do projeto — o endpoint já autentica por Bearer token, não por cookie, então não havia motivo pra restringir origem.
  - Validado com typecheck, build de produção e suíte completa.
