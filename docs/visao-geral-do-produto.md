...
- "Meu progresso" (`/statistics`, item "Progresso" da navegação): sequência de dias consecutivos, evolução do humor (gráfico dos últimos 30 dias a partir de `patient_mood_logs`, com média e tendência simples de alta/queda/estável), metas da semana com progresso real por meta (`GoalCard`, categoria/ícone/cor), visão geral de atividades (consultas, emergenciais, respiração, sons), atalhos para conquistas (com contagem real de desbloqueadas) e histórico completo, e lista de atividades recentes.
